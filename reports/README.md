...
The diagram above represents a system that integrates both local and cloud components to support our projects workflows, which are built around PyTorch models. Focusing on the local side of the diagram, the the big picture makes use of Cookiecutter and Docker. On one hand, Cookiecutter is used because it provides a template to organize the project, on the other hand, Docker makes sure that all dependencies are packaged in containers, which allows project to run consistently across different environments. That makes the project more reproducible. Focusing on the libraries requirements inside virtual environments, we used Python's AutoVen. With that, we made sure that the correct libraries and versions wereinstalled, so we avoided any possible dependancies issues. Concerning different configuration management, we decided to use Hydra, as it simplifies the handling of multiple configuration files, which is key to manage different setups in the project. The core machine learning work happens within PyTorch, which consumes local data stored in the system's local storage for model training and inference. DVC (Data Version Control) is used to version datasets and synchronize them between local and cloud environments, maintaining consistency across both setups.

On the cloud side, Git and GitHub Actions are integral for version control and continuous integration. GitHub Actions automates various tasks such as unit testing, dataset statistics checks, and linting whenever new code is pushed or merged, ensuring code quality. DVC facilitates data synchronization between the local machine and the cloud, making datasets version-controlled and accessible remotely. GCP (Google Cloud Platform) serves as the remote storage for data, and it also hosts the model. The trained model can be deployed using FastAPI, which creates an API endpoint that is hosted on Google Cloud Run, offering scalable and efficient model inference services. The system also integrates with GCP's Vertex AI for cloud-based training, where containerized versions of the model are pulled from the Container Registry and executed in the cloud. Additionally, GCP's logging services are used to monitor and log relevant information, such as errors and system logs, ensuring that any issues that arise during the deployment process are tracked and addressed.

Overall, this architecture has two main sides, the local and the cloud ones, which merge local development environments with scalable cloud-based deployment, creating a workflow for building, testing, and deploying our project.
